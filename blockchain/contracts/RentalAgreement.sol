...
contract RentalAgreement {
    ...
    function pay (uint256 deadline, uint256 nonce, uint256 value, Sign calldata cashierSign) payable public {
        ...
        if ((block.timestamp - rentStartTime_) > billingPeriodDuration_ && ((block.timestamp - rentStartTime_) / billingPeriodDuration_ - 1) * rentalRate_ > address(this).balance) revert("The contract is being in not allowed state");
        if (deadline < block.timestamp) revert("The operation is outdated");
